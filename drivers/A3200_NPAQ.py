...
from core.machine.drivers.A3200StatusItem import A3200StatusItem
...
if TYPE_CHECKING:
    from core.machine.axes.axis import Axis
...
class A3200_NPAQ(Driver):
    ...
    def auto_focus(self, axis:'Axis', on_off:bool, task:int|None=None):
        ...
        if self.A3200_is_open:
            ...
            return self.A3200_lib.A3200MotionAutoFocus(self.handle, task, axis.driver_index.value, on_off)
    ...
    def linear(self, axes:'list[Axis]', distances:list[float], task:int | None=None):
        ...
        '''
        if self.A3200_is_open:
            speeds = [self.default_motion_speed.value]*len(axes)
    ...
    def freerun(self, axis:'Axis',speed:float,task:int | None= None):
        if self.A3200_is_open:
            task = task if task is not None else self.task.value
            s = ct.c_double(speed)
            return self.A3200_lib.A3200MotionFreeRun(self.handle, task, axis.driver_index.value, s)
        
    def freerun_stop(self, axis:'Axis', task:int|None=None):
        if self.A3200_is_open:
            task = task if task is not None else self.task.value
            return self.A3200_lib.A3200FreeRunStop(self.handle,task,axis.driver_index.value)
    ...
    def absolute_move(self, axis:'Axis', position:float, speed: float | None=None, task:int | None=None):
        ...
        '''
        if self.A3200_is_open:
            if speed is None: speed = self.default_motion_speed.value
            #Convert python floats to c doubles
            p = ct.c_double*position
            s = ct.c_double*speed
            task = task if task is not None else self.task.value
            success = self.A3200_lib.A3200MotionMoveAbs(self.handle, axis.driver_index.value,p,s)
            if not success: 
                raise A3200Exception(source='A3200:linear',message=f'A3200 command fail {success}', level='estop')
    ...
    def incremental_move(self, axis:'Axis', position:float, speed: float | None=None, task:int | None=None):
        ...
        if self.A3200_is_open:
            ...
            s = ct.c_double*speed
            task = task if task is not None else self.task.value
            success = self.A3200_lib.A3200MotionMoveInc(self.handle, axis.driver_index.value,p,s)
            if not success: 
                raise A3200Exception(source='A3200:linear',message=f'A3200 command fail {success}', level='estop')
    ...
    def cmd_exe(self, command, task: int | None):
        ...
        if self.A3200_is_open:
            ...
            # result = ''
            # success = self.A3200_lib.A3200CommandExecute(self.handle, task, cmd, result)
    ...
    def sort_axes(self, axes: 'list[Axis]', distances: list[float], speeds:list[float]) -> 'tuple[list[Axis], list[float], list[float]]':
        ...
        grouped.sort(key=lambda x: x[0].driver_index.value)
...
# '''
    ...
    def get_status_item(self, item_code: A3200StatusItem | A3200AxisStatus, item_index:int, extra: int | None=None) -> float | None:
        ...
        if self.simulation.value or not self.A3200_is_open:
            return None
    ...
    def get_positions(self, axes:'list[Axis]') -> list[float] | None:
        if self.A3200_is_open and not self.simulation.value:
            num_items = len(axes)
            item_indices = [ax.driver_index.value for ax in axes]
    ...
    def get_position(self,axis:'Axis')->float:
        if self.A3200_is_open and not self.simulation.value:
            positions = self.get_status_item(item_code=A3200StatusItem.STATUSITEM_PositionFeedback,item_index=axis.driver_index.value)
            return positions

    def is_move_done(self,axis:'Axis',mode:str='done') -> float | None:
        if self.A3200_is_open and not self.simulation.value:
            move_done = self.get_status_item(item_code=A3200AxisStatus.AXISSTATUS_MoveDone, item_index=axis.driver_index.value)
            return move_done

    def set_absolute(self,task:int|None=None)->bool:
        if self.A3200_is_open and not self.simulation.value:
            task = task if task is None else self.task.value
